...
' original pit dimensions 6x13

' custom pit dimensions 9x20
' PLAYFIELD_WIDTH = 53, PLAYFIELD_HEIGHT = 20

Public Const PLAYFIELD_WIDTH As Integer = 30 '50
Public Const PLAYFIELD_HEIGHT As Integer = 14
...
TheShape2.Y = TheShape.Y
   ...
   For Y = 0 To MAX_SHAPE_HEIGHT - 1
     ...
     For X = 0 To MAX_SHAPE_WIDTH - 1
        TheShape2.Cell(X2, Y2) = TheShape.Cell(X, Y)
        Y2 = Y2 - 1
     Next X
       X2 = X2 + 1
   Next Y
...
Sub LoadMedia()
    ...
    Open "blobblob.ini" For Input As #1
...
Dim TypeAField(0 To PLAYFIELD_WIDTH - 1, 0 To PLAYFIELD_HEIGHT - 1) As Integer
Dim TypeBField(0 To PLAYFIELD_WIDTH - 1, 0 To PLAYFIELD_HEIGHT - 1) As Integer

Dim DeleteInfoField(1 To 10) As Integer
Dim Cell%, X%, Y%, ComboCount%, CellCount%
    ...
    For Y = PLAYFIELD_HEIGHT - 2 To 1 Step -1
      For X = LEFT_WALL_X + 1 To RIGHT_WALL_X - 1
         ...
         If StaticPlayField(X, Y) > 0 Then
            ...
            Cell = StaticPlayField(X, Y)
                                    
            ' ** top **
            If Cell = StaticPlayField(X, Y - 1) Then
              CellCount = CellCount + 1
            End If
            
            ' ** right **
            If Cell = StaticPlayField(X + 1, Y) Then
              CellCount = CellCount + 1
            End If
    ...
    For Y = PLAYFIELD_HEIGHT - 2 To 1 Step -1
      For X = LEFT_WALL_X + 1 To RIGHT_WALL_X - 1
         ...
         If TypeAField(X, Y) > 0 Then
            ...
            ' ** top **
            If Cell = TypeAField(X, Y - 1) Then
              CellCount = CellCount + 1
            End If
            
            ' ** right **
            If Cell = TypeAField(X + 1, Y) Then
              CellCount = CellCount + 1
            End If
    ...
    Next Y
     ...
     '** divide the count of Type B cells by 2 to get the ComboCount **
     
    ComboCount = 0
    
    For Y = PLAYFIELD_HEIGHT - 2 To 1 Step -1
      For X = LEFT_WALL_X + 1 To RIGHT_WALL_X - 1
          If TypeBField(X, Y) > 0 Then
    ...
    For Y = PLAYFIELD_HEIGHT - 2 To 1 Step -1
      For X = LEFT_WALL_X + 1 To RIGHT_WALL_X - 1
         ...
         If TypeBField(X, Y) > 0 Then
            ...
            ' ** top **
            If Cell = StaticPlayField(X, Y - 1) Then
              TypeBField(X, Y - 1) = Cell
            End If
            
            ' ** right **
            If Cell = StaticPlayField(X + 1, Y) Then
              TypeBField(X + 1, Y) = Cell
            End If
    ...
    For Y = PLAYFIELD_HEIGHT - 2 To 1 Step -1
      For X = LEFT_WALL_X + 1 To RIGHT_WALL_X - 1
          ...
          DeletePlayField(X, Y) = TypeBField(X, Y)
          If DeletePlayField(X, Y) Then StaticPlayField(X, Y) = 0
      Next X
    Next Y
...
Public Sub DisplayGameInfo()
 ...
 Bitmap_DrawText "High Score", RIGHT_WALL_X * W + 7 * W, H * 1
 Bitmap_DrawText HighScore, RIGHT_WALL_X * W + 7 * W, H * 2
 
 Bitmap_DrawText "Current Score", RIGHT_WALL_X * W + 7 * W, H * 3
 Bitmap_DrawText CurrentScore, RIGHT_WALL_X * W + 7 * W, H * 4
 
 Bitmap_DrawText "Current Speed", RIGHT_WALL_X * W + 7 * W, H * 5
 Bitmap_DrawText Trim(Str(CurrentSpeed)) + Dstr, RIGHT_WALL_X * W + 7 * W, H * 6
 
 Bitmap_DrawText "Cleared", RIGHT_WALL_X * W + 7 * W, H * 7
 Bitmap_DrawText CurrentShapesClearedCount, RIGHT_WALL_X * W + 7 * W, H * 8
 
 Bitmap_DrawText "Next", RIGHT_WALL_X * W + 7 * W, H * 9
 ...
 H = CellTextHeightPixels
  ...
  Select Case GameState
     ...
     Case GAME_STATE_BEGIN:
          Bitmap_DrawText "Welcome", LEFT_WALL_X * W + 5 * W, H * 6
          Bitmap_DrawText "  to   ", LEFT_WALL_X * W + 5 * W, H * 7
          Bitmap_DrawText "Blob Blob", LEFT_WALL_X * W + 5 * W, H * 8
          
     Case GAME_STATE_PAUSED:
          Bitmap_DrawText " Paused ", LEFT_WALL_X * W + 5 * W, H * 6
          
     Case GAME_STATE_GAME_OVER:
          Bitmap_DrawText "  Game  ", LEFT_WALL_X * W + 5 * W, H * 6
          Bitmap_DrawText "  Over  ", LEFT_WALL_X * W + 5 * W, H * 7
...
For i = 1 To 10
  For X = LEFT_WALL_X + 1 To RIGHT_WALL_X - 1
    For Y = PLAYFIELD_HEIGHT - 2 To 1 Step -1
       ...
       If StaticPlayField(X, Y) = 0 And DeletePlayField(X, Y) = 0 Then
